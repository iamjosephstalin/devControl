...
import { DeploymentType, DEPLOYMENT_TYPES, INTEGRATION_CONFIG, createIntegration } from "@/lib/integrations"
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent className="max-w-md max-h-[90vh] overflow-hidden">
        <DialogHeader>
          <DialogTitle>Connect Deployment Platform</DialogTitle>
          <DialogDescription>
            Connect your hosting platform to manage deployments and services
          </DialogDescription>
        </DialogHeader>
        
        <div className="space-y-4 max-h-[60vh] overflow-y-auto">
          <div>
            <Label htmlFor="type">Deployment Platform</Label>
            <Select onValueChange={(value) => setType(value as DeploymentType)}>
              <SelectTrigger className="w-full">
                <SelectValue placeholder="Choose a hosting platform" />
              </SelectTrigger>
              <SelectContent>
                {DEPLOYMENT_TYPES.map((key) => {
          ...
          {config && (
            <>
              ...
              <div className="text-sm text-muted-foreground p-3 bg-muted/50 rounded-md">
                <strong>Need help?</strong> Create credentials at{" "}
                <a
        ...
        <div className="flex flex-col sm:flex-row gap-2 pt-4 border-t">
          <Button
            ...
            className="flex-1"
          >
            {isCreating ? "Connecting..." : "Connect Platform"}
          </Button>
